...
service /oauth2 on new http:Listener(9090) {
    ...
    isolated resource function post token(@http:Header{name:http:AUTHORIZATION} string authorization, @http:Payload string payload) returns TokenResponse|http:BadRequest|error {
        if payload is string {
            ...
            if grantType == "client_credentials" {
                return handleClientCredentialGrant(authorization);
            }
            return <http:BadRequest>{body: "Invalid grant type"};
        }
    }    
}

isolated function handleClientCredentialGrant(string authorization) returns TokenResponse|http:BadRequest|error {
